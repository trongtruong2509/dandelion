...
import { fetchCurrentPlaylistInfo, addTrackToPlaylist, removeTrackFromPlaylist } from "../../slices/playlistSlice";
            ...
            <div>
               {currentPlaylist?.songs?.length > 0 ? (
                  <>
                     ...
                     <div className="flex w-full gap-2 mt-1 text-xs text-secondary">
                        <p>
                           {currentPlaylist?.songs.length > 1 ? `${currentPlaylist?.songs.length} tracks` : "1 track"}
                        </p>
               ...
               {currentPlaylist?.createdBy === user?.id && currentPlaylist?.songs?.length < 20 && (
                  <div className="mt-5">
                     ...
                     <div className="mt-5">
                        {suggestSongs?.map((s) => (
                           <SongItem key={s.id} info={s} playlistMode addPlaylist onAdd={handleAddToPlaylist} />
                        ))}
                     </div>
                  </div>
               )}
